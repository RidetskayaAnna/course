...
namespace Agent.Form
{
    public partial class Work : UserControl
    {
        ...
        public void Work_load()
        {
            ...
            SqlDataAdapter command = new SqlDataAdapter($@"Select idwork, name as [Отдел] from work", sqlConnection);
        ...
        public void comboBoxposition()
        {
            sqlConnection.Close();
            sqlConnection.Open();
            string query = "Select Distinct name from post";
            SqlDataAdapter sqlDbDataAdapter = new SqlDataAdapter(query, sqlConnection);
            DataSet dataSet = new DataSet();
            sqlDbDataAdapter.Fill(dataSet);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                button6.Width = 377;
                button6.Left = 70;
                button3.Text = "Редактировать отдел";
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                button6.Width = 420;
                button6.Left = 55;
                button3.Text = "Вывод отделов";
                button6.Text = "Вывод отделов и должностей";
                panel3.BringToFront();
                dataGridView1.Enabled = false;
                dataGridView2.Enabled = false;
            }
            ...
            try
            {
                if (label6.Text == "Добавить отдел")
                {
                    if (textBox3.Text != "")
                    {
                        ...
                        {
                            MessageBox.Show("Такой отдел уже есть!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
        ...
        public void Position_load()
        {
            sqlConnection.Open();
            DataSet dataSet = new DataSet();
            SqlDataAdapter command = new SqlDataAdapter($@"Select idpost, post.name as Должность ,post.idwork from post inner join work on post.idwork=work.idwork where post.idwork={id}", sqlConnection);
            command.Fill(dataSet);
        ...
        {
            if (button3.Text == "Добавить отдел")
            {
                clear();
                label6.Text = "Добавить отдел";
            ...
            else if (button3.Text == "Удалить отдел")
            {
                clear();
                if (id != 0)
                {
                    try
                    {
                        if (MessageBox.Show($@"Вы уверены что хотите удалить отдел {dataGridView1.CurrentRow.Cells[1].Value.ToString()}?", "Удаление", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
            ...
            else if (button3.Text == "Вывод отделов")
            {
                dataGridView1.Enabled = true;
                dataGridView2.Enabled = true;
                clear();
                ...
                ExcelApp.Rows[visible + 2].Columns[1] = "Ридецкая Анна Михайловна";
                for (int i = 1; i < dataGridView1.Columns.Count; i++)
                {
                    ExcelApp.Cells[1, i] = dataGridView1.Columns[i].HeaderText;
                ...
                for (int j = 1; j < dataGridView1.ColumnCount; j++)
                {
                    for (int i = 0; i < visible; i++)
                    {
                        ...
                        {
                            ExcelApp.Cells[i + 2, j] = dataGridView1.Rows[i].Cells[j].Value.ToString();
                        }
            ...
            else
            {
                ...
                {
                    clear();
                    label6.Text = "Редактировать Отдел";
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            else if (button6.Text == "Вывод отделов и должностей")
            {
                ...
                panel3.Visible = false;
                visible = 0;
                string query31 = $@"Select Count(idpost) from post inner join work on post.idwork=work.idwork";
                ...
                ExcelWorkSheet.StandardWidth = 30;
                ExcelWorkSheet.Columns.ColumnWidth = 20;
                ExcelApp.Rows[1].Columns[2] = "Должности и отделы";
                ExcelApp.Rows[visible + 3].Columns[2] = "Ридецкая Анна Михайловна";
                ExcelApp.Cells[2, 1] = "Должность";
              
                ExcelApp.Cells[2, 2] = "Отдел";

                for (int j = 1; j < Convert.ToInt32(row22[column22].ToString()) + 1; j++)
                {
                    string query3 = $@"Select idpost, post.name as Должность , work.name from post right join work on post.idwork=work.idwork order by work.name ";
                ...
                for (int i = 0; i < visible; i++)
                {
                    ...
                    {
                        Microsoft.Office.Interop.Excel.Range range = ExcelWorkSheet.Range[$"A1:B{visible + 3}"];
                        ExcelWorkSheet.Range[$"A1:B{visible + 3}"].Cells.WrapText = true;
                        range.Borders.LineStyle = Excel.XlLineStyle.xlContinuous;
                        range.Borders.Weight = Excel.XlBorderWeight.xlMedium;
                    }
                }
            ...
            else
            {
                if (id2 != 0)
                {
                    ...
                    comboBox2.SelectedValue = dataGridView2.CurrentRow.Cells[2].Value.ToString();
                }
                else
                {
                    MessageBox.Show("Строка не выбрана!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            char c = e.KeyChar;
            e.Handled = !((c >= 'а' && c <= 'я') || (c >= 'А' && c <= 'Я') || (c == 'ё' || c == 8 || c == 32 || c == '0' || c == '1' || c == '2' || c == '3' || c == '4' || c == '5' || c == '6' || c == '7' || c == '8' || c == '9'));
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            ...
            //{
                if (button6.Text == "Добавить должность")
                {
                    if (comboBox1.Text != "" && comboBox2.SelectedIndex != -1)
                    {
                        k = 0;
                        string query31 = $@"Select Count(idpost) from post inner join work on post.idwork=work.idwork";
                        DataTable data31 = new DataTable();
                        ...
                        for (int i = 0; i < Convert.ToInt32(row22[column22].ToString()); i++)
                        {
                            string query3 = $@"Select idpost, post.name as Должность ,post.idwork from post inner join work on post.idwork=work.idwork";
                            DataTable data3 = new DataTable();
                            SqlDataAdapter command3 = new SqlDataAdapter(query3, sqlConnection);
                            command3.Fill(data3);
                            DataColumn column2 = data3.Columns[1];
                            ...
                            string name = (row2[column2].ToString());
                            DataColumn column12 = data3.Columns[2];
                        ...
                        {
                            sqlConnection.Open();
                            SqlCommand command = new SqlCommand($@"INSERT INTO [post](name,idwork) VALUES (@n,@w);", sqlConnection);
                ...
                else
                {
                    if ( comboBox1.Text != "" && comboBox2.SelectedIndex != -1)
                    {
                        ...
                        if (k == 0 || j == id)
                        {
                            k = 0; j = 0;
                            sqlConnection.Open();
                            SqlCommand command = new SqlCommand($@"UPDATE post SET name=@n,idwork=@w WHERE idpost=@id", sqlConnection);
                            command.Parameters.AddWithValue("@n", (comboBox1.Text));
                            command.Parameters.AddWithValue("@w", (comboBox2.SelectedValue));
